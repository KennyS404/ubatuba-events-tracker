...
const RegisterForm: React.FC<RegisterFormProps> = ({ darkMode }) => {
  ...
  const validateForm = () => {
    ...
    if (!emailRegex.test(formData.email)) {
      setErrorMessage(t('register.validEmail'))
      return false
    }
    ...
    if (formData.password !== formData.confirmPassword) {
      setErrorMessage(t('register.passwordsMatch'))
      return false
    }
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMessage(error.response?.data?.detail || t('register.registerError'))
    } finally {
        ...
        <div className="p-8 pt-12 relative z-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input
                ...
                placeholder={t('register.usernamePlace')}
                required
              />
            </div>

            <div>
              <label htmlFor="email" className={`block text-sm font-medium mb-2 ${darkMode ? 'text-blue-200' : 'text-blue-700'}`}>
                {t('register.email')}
              </label>
              <input
                id="email"
            ...
            <div>
              <label htmlFor="fullName" className={`block text-sm font-medium mb-2 ${darkMode ? 'text-blue-200' : 'text-blue-700'}`}>
                {t('register.fullName')}
              </label>
              <input
                id="fullName"
                ...
                placeholder={t('register.fullNamePlace')}
              />
            </div>
            ...
            <div>
              <label htmlFor="confirmPassword" className={`block text-sm font-medium mb-2 ${darkMode ? 'text-blue-200' : 'text-blue-700'}`}>
              ...
              <input
                ...
                placeholder={t('register.confirmPasswordPlace')}
                required
              />
            </div>
            ...
            <motion.button
              ...
              className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-4 rounded-lg transition-colors focus:outline-none focus:ring-4 focus:ring-blue-300 disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center shadow-md mt-6"
            >
              {isSubmitting ? (
          ...
          <div className="mt-6 text-center">
            <p className={`${darkMode ? 'text-blue-200' : 'text-blue-600'}`}>
              ...
              <Link to="/login" className={`${darkMode ? 'text-white' : 'text-blue-700'} hover:underline font-medium`}>
                {t('app.login')}
              </Link>
            </p>
          </div>
